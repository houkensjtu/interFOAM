...
:
    ...
    //-Add temperature, read cp and Prandtl number
    ...
    Pr1_("Pr", dimless, nuModel1_->viscosityProperties().lookup("Pr")),
    Pr2_("Pr", dimless, nuModel2_->viscosityProperties().lookup("Pr")),
    //-Add temperature, read cp and Prandtl number 
...
// //- Add temperature
 Foam::tmp<Foam::surfaceScalarField> Foam::twoPhaseMixture::kappaf() const
 {
     const surfaceScalarField alpha1f
     (
         min(max(fvc::interpolate(alpha1_), scalar(0)), scalar(1))
     ...
     (
         ...
         (
             "kappaf",            
             alpha1f*cp1_*rho1_*(1/Pr1_)*fvc::interpolate(nuModel1_->nu()) + (scalar(1) - alpha1f)*cp2_*rho2_*(1/Pr2_)*fvc::interpolate(nuModel2_->nu())
...
//- Add temperature
Foam::tmp<Foam::volScalarField> Foam::twoPhaseMixture::kappa() const
{
    ...
    (
        ...
        (
            "kappa",            
            limitedAlpha1*cp1_*rho1_*(1/Pr1_)*nuModel1_->nu() + (scalar(1) - limitedAlpha1)*cp2_*rho2_*(1/Pr2_)*nuModel2_->nu()
        )
    );
}
//- Add temperature
